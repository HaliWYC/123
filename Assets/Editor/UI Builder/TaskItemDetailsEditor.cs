...
public class TaskItemDetailsEditor : EditorWindow
{
    [SerializeField]
    ...
    private TaskItemDetailsList_SO database;
    ...
    private TaskItemDetails activeItem;

    private VisualElement iconPreview;

    private Sprite defaultIcon;

    private List<TaskItemDetails> itemList = new List<TaskItemDetails>();

    [MenuItem("山海：绝命孤城/TaskItemDetailsEditor")]
    public static void ShowExample()
    {
        TaskItemDetailsEditor wnd = GetWindow<TaskItemDetailsEditor>();
        wnd.titleContent = new GUIContent("TaskItemDetailsEditor");
    }
    ...
    {
        TaskItemDetails newItem = new TaskItemDetails();
    ...
    {
        var dataArray = AssetDatabase.FindAssets("TaskItemDetailsList_SO");

        if (dataArray.Length > 1)
        {
            var path = AssetDatabase.GUIDToAssetPath(dataArray[0]);
            database = AssetDatabase.LoadAssetAtPath(path, typeof(TaskItemDetailsList_SO)) as TaskItemDetailsList_SO;
        }

        itemList = database.taskItemDetailsList;
    ...
    {
        activeItem = (TaskItemDetails)selectedItem.First();
        getItemDetails();
