...
public class OtherItemDetailsEditor : EditorWindow
{
    [SerializeField]
    ...
    private OtherItemDetailsList_SO database;
    ...
    private OtherItemDetails activeItem;

    private VisualElement iconPreview;

    private Sprite defaultIcon;

    private List<OtherItemDetails> itemList = new List<OtherItemDetails>();

    [MenuItem("山海：绝命孤城/OtherItemDetailsEditor")]
    public static void ShowExample()
    {
        OtherItemDetailsEditor wnd = GetWindow<OtherItemDetailsEditor>();
        wnd.titleContent = new GUIContent("OtherItemDetailsEditor");
    }
    ...
    {
        OtherItemDetails newItem = new OtherItemDetails();
    ...
    {
        var dataArray = AssetDatabase.FindAssets("OtherItemDetailsList_SO");

        if (dataArray.Length > 1)
        {
            var path = AssetDatabase.GUIDToAssetPath(dataArray[0]);
            database = AssetDatabase.LoadAssetAtPath(path, typeof(OtherItemDetailsList_SO)) as OtherItemDetailsList_SO;
        }

        itemList = database.otherItemDetailsList;
    ...
    {
        activeItem = (OtherItemDetails)selectedItem.First();
        getItemDetails();
    ...
    private void getItemDetails()
    {
        ...
        itemDetailsSection.Q<EnumField>("ItemQuality").Init(activeItem.OtherItemQuality);
        itemDetailsSection.Q<EnumField>("ItemQuality").value = activeItem.OtherItemQuality;
        itemDetailsSection.Q<EnumField>("ItemQuality").RegisterValueChangedCallback(evt =>
        {

            activeItem.OtherItemQuality = (BasicQualityType)evt.newValue;
            itemListView.Rebuild();
        });

        itemDetailsSection.Q<EnumField>("ConsumeType").Init(activeItem.otherItemType);
        itemDetailsSection.Q<EnumField>("ConsumeType").value = activeItem.otherItemType;
        itemDetailsSection.Q<EnumField>("ConsumeType").RegisterValueChangedCallback(evt =>
        {

            activeItem.otherItemType = (OtherItemType)evt.newValue;
            itemListView.Rebuild();
        });
